              ...
              <div class="form-group">
                ...
                <select class="form-control <?= (session('errors.id_obat')) ? 'is-invalid' : '' ?>" id="id_obat" name="id_obat" required>
                  ...
                  <?php foreach ($obatList as $obat) : ?>
                    <option value="<?= $obat['id_obat'] ?>" data-nama="<?= $obat['nama_obat'] ?>" data-satuan="<?= $obat['satuan'] ?>">
...
<script>
  $(document).ready(function() {
    // Populate obat data when selected
    ...
    // Set initial values if returning from failed validation
    if('<?= old('id_obat') ?>') {
